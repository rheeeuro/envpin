...
test('packaged extension: lifecycle, clipboard, scrolling, two-profile transport, deletion and restart', async () => {
  ...
  try {
    ...
    const managerPromise = a.context.waitForEvent('page');
    await page.getByRole('link', { name: 'Manage' }).click();
    const manager = await managerPromise;
    await expect(manager).toHaveURL(new RegExp(`chrome-extension://${a.id}/manage.html`));
    for (const name of ['Personal demo', 'Development demo', 'Staging demo', 'Sandbox demo']) await add(manager, name);
    await manager.getByRole('button', { name: 'Pin Example API / Staging demo' }).click();
    await expect(manager.locator('article').first()).toContainText('Staging demo');
    await manager.getByLabel('Drag Example API / Sandbox demo').dragTo(manager.locator('article').filter({ hasText: 'Personal demo' }));
    await expect(manager.locator('article').nth(1)).toContainText('Sandbox demo');
    await page.bringToFront();
    await expect(page.locator('article').first()).toContainText('Staging demo');
    await expect(page.locator('article').nth(1)).toContainText('Sandbox demo');
    ...
    const otherManagerPromise = b.context.waitForEvent('page');
    await other.getByRole('link', { name: 'Manage' }).click();
    const otherManager = await otherManagerPromise;
    await otherManager.getByRole('button', { name: 'Delete Example API / Personal demo' }).click();
    await otherManager.getByRole('dialog').getByRole('button', { name: 'Delete', exact: true }).click();
    await expect(otherManager.getByText('Personal demo', { exact: true })).toHaveCount(0);
